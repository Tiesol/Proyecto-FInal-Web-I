...
function generarBotones(proyecto) {
    ...
    if (proyecto.workflow_state_id === 2) {
        ...
        botones = botones + '<button class="btn btn-rechazar" onclick="rechazarDirecto(' + proyecto.id + ')">Rechazar</button>';
    }
...
function mostrarDetalle(campania) {
    ...
    if (campania.workflow_state_id === 2 || campania.workflow_state_id === 3) {
        ...
        html += '<button class="btn btn-rechazar" onclick="rechazarDirecto(' + campania.id + ')">Rechazar</button>';
        html += '</div>';
    }
...
function rechazarDirecto(id) {
    var token = localStorage.getItem('token');

    fetch(API_URL + '/admin/campaigns/' + id + '/reject', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer ' + token
        }
    ...
    .then(function(data) {
        mostrarMensaje('Proyecto rechazado correctamente', 'exito');
        if (campaniaActual) {
            volverALista();
        } else {
            cargarProyectos();
    ...
    .catch(function(error) {
        console.error('Error:', error);
        mostrarMensaje('Error al rechazar el proyecto', 'error');
    });
}
